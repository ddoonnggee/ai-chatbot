...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log('Embed API 请求参数:', { token: token?.substring(0, 20) + '...' });

        if (!token) {
            return NextResponse.json(
                { success: false, error: 'Token 参数缺失' },
                { status: 400 }
            );
        }
        ...
        try {
            ...
            console.log('解码后的Token信息:', decodedPayload);
            ...
            if (!finalHostDomain) {
                ...
                return NextResponse.json(
                    { success: false, error: `缺少宿主域名信息: ${finalHostDomain}` },
                    { status: 403 }
                );
            }
            ...
            if (decodedPayload.userId) {
                try {
                    console.log('开始查询/创建用户:', decodedPayload.userId);
                    ...
                    if (existingUserApp) {
                        ...
                        user = await getUserByEmail(`${decodedPayload.appId}.${decodedPayload.userId}@ai.app`);
            ...
            //             appId: decodedPayload.appId,
            //             userId: decodedPayload.userId,
            //             domain: appConfig.allowedDomains,
            ...
            return NextResponse.json(
                { success: false, error: 'Token无效或已被篡改' },
                { status: 401 }
            );
        }

    } catch (error) {
        console.error('Embed API 处理失败:', error);
